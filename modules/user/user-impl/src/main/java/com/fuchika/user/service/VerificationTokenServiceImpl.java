...
import com.fuchika.user.exception.*;
import com.fuchika.user.model.User;
import com.fuchika.user.model.VerificationToken;
import com.fuchika.user.model.VerificationTokenType;
...
public class VerificationTokenServiceImpl extends BaseService implements VerificationTokenService {
	...
	public VerificationToken sendEmailRegistrationToken(String userId) throws UserNotFoundException {
		User user = ensureUserIsLoaded(userId);
		VerificationToken token = new VerificationToken(user, VerificationTokenType.emailRegistration,
	...
	public VerificationToken sendLostPasswordToken(LostPasswordRequest lostPasswordRequest) throws ValidationException {
		...
		if (user != null) {
			List<VerificationToken> tokens = tokenRepository.findByUserIdAndTokenType(user.getId(),
					VerificationTokenType.lostPassword);
			token = getActiveToken(tokens);
			if (token == null) {
				token = new VerificationToken(user, VerificationTokenType.lostPassword,
						lostPasswordTokenExpiryTimeInMinutes);
				tokenRepository.save(token);
			}
	...
	public VerificationToken generateEmailVerificationToken(String emailAddress)
		...
		VerificationToken token = getActiveToken(
				tokenRepository.findByUserIdAndTokenType(user.getId(), VerificationTokenType.emailVerification));
		if (token == null) {
	...
	private User ensureUserIsLoaded(String userIdentifier) throws UserNotFoundException {
		User user = null;
